    ...
    *,
    ...
    if probe.serving:
        ...
        return head + (
            ...
            f"DO NOT RETRY BLIND — A TIMEOUT HERE DOES NOT MEAN THE {verb.upper()} "
            f"DID NOT HAPPEN. Lifecycle routes perform the whole operation "
            f"synchronously and only then answer, so the response time IS the "
            f"operation time: a SUCCESSFUL restart has been measured at 77s on "
            f"this fleet, and a cold boot runs past the 240s boot-drain window "
            f"— longer than any client timeout. The likeliest reading of this "
            f"timeout is that the work is still running, or already finished.\n"
            f"VERIFY BEFORE RETRYING: check whether {name!r} came up — "
            f"`inbox_subscribers` / `session_id` on its peer row, or "
            f"`sac agents list`. If it is up, the {verb} SUCCEEDED and only the "
            f"response was lost. Retrying then tears down the session the first "
            f"call just created, and an agent mid-turn loses that turn.\n"
            f"If it did NOT come up, report {route} with these two timings."
        )
    return (
