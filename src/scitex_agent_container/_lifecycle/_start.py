...
def _announce_start_verdict(verdict) -> None:
    ...
    if verdict.is_unknown:
        system_msg(
            f"cannot confirm '{verdict.agent}' is alive — starting it. "
            f"Not destructive: the runtime's duplicate-session guard no-ops "
            f"if it turns out to be alive (only `--force` tears a session down).",
            style="warn",
        )
