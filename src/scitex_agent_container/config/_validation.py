...
# The spec-key inventory (_KNOWN_SPEC_KEYS + the v3 relocated/removed
# tables) lives in the sibling ``_spec_keys`` module under the per-file
# line cap; re-exported so existing imports keep resolving.
from ._spec_keys import (  # noqa: E402
    _KNOWN_SPEC_KEYS,
...
def validate_raw(raw: dict, path: str) -> list[str]:
    ...
    if not isinstance(spec, dict):
        ...
        # spec.engines — the MULTI-backend surface. Everything decidable
        # from the spec TEXT (two defaults, no default, unknown harness,
        # malformed provider, a legacy single-backend block that
        # DISAGREES with the default engine) is a load error here; the
        # host-dependent half (unset $API_KEY, an endpoint that will not
