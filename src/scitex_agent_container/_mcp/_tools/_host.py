...
def register_host_tools(mcp) -> None:
    @mcp.tool()
    def sac_host_show() -> dict[str, Any]:
    ...
    @mcp.tool()
    def sac_host_probe(peer: str, timeout: int = 5) -> dict[str, Any]:
    ...
    @mcp.tool()
    def sac_host_exec(peer: str, command: list[str]) -> dict[str, Any]:
