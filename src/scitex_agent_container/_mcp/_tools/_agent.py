...
def agent_twin(
    ...
    name + fresh a2a port; at first boot the host resumes the parent's live
    session with ``--fork-session``, so the twin carries the context but
    writes to a session of its own. (Use one to inherit context without
    sharing future context, split parallel work, or run heavy work off your
    main loop; a plain Task subagent is cheaper otherwise.)
    ...
    attribute to it), verified by a boot gate that REFUSES to start a twin
    whose injected identity is missing or is its parent's. OWNER = parent,
    but scitex-todo cannot default the card owner from env, so the twin MUST
    pass ``assignee=<parent>`` (== ``$SAC_TWIN_PARENT``) on every card write
    — a hard rule, not an env guarantee; an ephemeral twin that owns cards
    then exits orphans them.

    ``tag`` (PREFERRED) is a descriptive slug giving the DETERMINISTIC id
    ``<parent>-forked-<tag>``: re-running the same tag targets the SAME twin
    instead of minting a new one, and it is addressable by a name you already
    know. ``name`` sets an explicit id and excludes ``tag``; omit both for
    the legacy ``<parent>-twin`` (bumped if taken). ``persist`` makes it
    long-lived (default ephemeral); ``task``/``role``/``caller`` optional.
    Returns ``{"status":"ok","twin":..,"result":{..}}`` else
    ``{"status":"error","reason":..}``.
    """
    from ..._lifecycle._spawn_client import SpawnRequestError, request_spawn
    ...
    try:
        twin_name, doc = prepare_twin_spawn(
            parent, twin_name=name, tag=tag, task=task, persist=persist, role=role
        )
