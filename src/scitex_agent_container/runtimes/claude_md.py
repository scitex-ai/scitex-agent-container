...
#: Hard budget for the launch-time orientation block (heading included).
#: Details belong in ``sac whoami`` — this block must stay scannable.
ORIENTATION_MAX_LINES = 12


def build_orientation_lines(config: AgentConfig) -> list[str]:
    """Launch-time orientation: where / how-run / role, in ≤12 lines.

    Generated HOST-side from the loaded spec (richer than the agent's
    in-container view — intended). The full self-serve view lives in
    ``sac whoami``; this block only orients a fresh session. Keys are
    lowercase ``key=value`` tokens so the block stays greppable and
    compact.
    """
    # stx-allow: fallback (reason: hostname resolution reads config.yaml /
    # scitex_config; CLAUDE.md generation must never fail on a degraded
    # host — fall back to the raw short hostname.)
    try:
    ...
    image = Path(config.image).name if config.image else "(default sac SIF)"
    model = config.claude.model or config.model
    role = config.env.get("SCITEX_AGENT_CONTAINER_ROLE", config.labels.get("role", ""))
    purpose = config.labels.get("purpose", "")

    lines = [
        "### Orientation",
        f"- where: host={host} image={image} workdir={config.expanded_workdir}",
        f"- run-as: runtime={config.runtime} model={model} "
        f"restart={config.restart.policy}",
        f"- role: {role or '(unset)'}; purpose: {purpose or '(unset)'}",
        "- Full in-container view: run `sac whoami` (`--json` for machine form).",
        "- Skills: scitex-agent-container "
        "(~/.claude/skills/scitex/scitex-agent-container/).",
    ]
    # Enforce the budget structurally (a future edit that grows this list
    # past the cap should fail tests, not quietly bloat every CLAUDE.md).
    assert len(lines) <= ORIENTATION_MAX_LINES, "orientation block over budget"
    return lines
...
def setup_claude_md(config: AgentConfig, workdir: str) -> None:
    ...
    lines.extend(build_orientation_lines(config))
    lines.append("")
