...
| `provider` | string OR `{ name, auth_token }` OR `{ base_url, auth_token_env }` | Point the SDK at any Anthropic-compatible backend. Registered names may use the compact string form (`provider: mimo`) or a declarative mapping such as `{name: codex, auth_token: auto}`. `auto` defers secret resolution/generation until real launch, keeping help/explain/dry-run secret-free; literal tokens belong only in private, untracked specs. **Legacy dict shape** remains accepted: `base_url` endpoint + `auth_token_env` NAME of the host env var holding the key. Mutually exclusive with `account`; relaxes the `claude-*` model-alias check. Auto-injects `ANTHROPIC_MODEL` from `spec.claude.model`. Adding a provider = add `{base_url, auth_token_env}` to `PROVIDERS` in `_provider_registry.py`. See ADR-0011. |
