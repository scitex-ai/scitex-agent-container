...
4. Writes a sac registry entry so `sac list` / `sac stop` / `sac agent logs` / `sac attach` route correctly.
...
| `sac attach` exits immediately | session was killed externally; check `sac agent logs` for crash trace |
