...
| Command | Purpose |
|---|---|
| `sac agents create <name> --template <kind> --project <p> [--agent-id <id>]` | Stamp a fresh agent spec from a `_template_<kind>/` dir under the agents root (`--template` fails loud with the live-scanned choice list on an unknown kind — currently `python_developer`/`researcher`/`generalist`; `--project` is only required when the chosen dir-template carries the `SAC_PLACEHOLDER_PROJECT` token). Renamed from `new` (2026-07, PR #535); folds in the old narrower `create --template developer\|scientist` (PR #534), now removed. Built-in inline kinds `minimal`/`full` (no `--project` needed) also still work. NOT the same "templates" as [08_templates.md](08_templates.md) (that's `config/examples/agent-templates/` deployment-pattern YAML samples — an unrelated, pre-existing naming collision). |
| `sac agents start <agent> [--foreground] [-v/--verbose]` | Start the agent. Daemon by default; `--foreground` streams stdio + blocks. On an interactive terminal launch (no `--yes`/`-y`, no `--dry-run`/`--json`/`--foreground`/`--one-shot`/`--broker-self`), previews the plan then refuses — a short summary by default, or the FULL plan (`-v`/`--verbose`; mounts/env/hooks/skills, same detail as `sac agents explain`) — and requires `--yes`/`-y` to actually launch; non-interactive callers (scripts, cron, the supervisor) proceed without a prompt. Honors `spec.host` / `spec.hosts` (cross-host placement; see [11_remote-deploy.md](11_remote-deploy.md)) and `spec.a2a.port` (HTTP inbound). |
