...
2. Inject the **path** via port 1 or 2 (e.g. `FLEET_GITEA_TOKEN_PATH=~/.bash.d/secrets/.../${SAC_NAME}.gitea-token`), or read the file in the parent shell and inject the value via `${VAR}`.
