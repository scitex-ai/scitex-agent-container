...
| 2. **MCP server env** | `dot_claude/.mcp.json` → `mcpServers.<name>.env` | only the spawned MCP server process | no | MCP launcher reads it directly |
| 3. **Workspace dotenv** | `dot_claude/.env` → `{workdir}/.env` | anything the agent (or its subprocesses) chooses to source | **yes — file in workdir** | `runtimes/_dot_claude.py::deploy_dot_claude` |
...
The `dot_claude/` directory (override path with `spec.dot_claude:`) is sac's generic file-deploy convention: each file/subdir inside is materialized into the workspace at agent start, with `${VAR}` and `${metadata.name}` interpolation. sac does not know about any specific project — projects contribute the content.

## Decision tree

- Need it inside Claude's shell for the running session? → **port 1** (`spec.env`).
- Need it inside an MCP server? → **port 2** (`dot_claude/.mcp.json` env).
- Need it available to *any* subprocess the agent spawns later (cron, ssh-launched commands, fresh shells)? → **port 3** (`dot_claude/.env` → workspace `.env`).
...
`dot_claude/.mcp.json` env values use the same `${VAR}` syntax (resolved by the MCP launcher in the parent shell at spawn time).
