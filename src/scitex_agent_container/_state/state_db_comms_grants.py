"""Explicit cross-group send permissions (``comms_grants`` table).
...
The ``sender`` identity is authenticated by
:class:`_listen._acl.NodeAuthMiddleware` resolving the bearer; the
optional ``note`` on :func:`grant_send` is a free-form audit
annotation (e.g. the ticket / handoff that authorised the grant).
"""
