#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# File: src/scitex_agent_container/_state/state_store_grants_store.py
"""The ``comms_grants`` store — schema, connection, and the shared handle.

Split out of :mod:`.state_store_grants` (which stays the import surface
:mod:`.state_store_nodes` re-exports) the same way
:mod:`.state_store_comms_nodes_store` is split out of its sibling. Nothing here
...
===================================================================
``state_store_grants._open`` is imported by name by
...
#:
#: NOT a tuning knob, for the reason :mod:`.state_store_comms_nodes_store`
...
def run_with_reconnect(operation: "Callable[[Store], Any]") -> Any:
    ...
    :mod:`.state_store_grants_rename` is idempotent in the same way: with
...
def new_grants_store() -> "Store":
    ...
    bulk-imported from peers by ``state_store_export``. Under SINGLE_WRITER the
