#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# File: src/scitex_agent_container/_state/state_store_gc.py
"""Garbage-collection sweep for ``instances`` records.

On the shared PostgreSQL store since 2026-08-28 (:mod:`.state_store_instances`);
``db_path`` is gone from the signature because there is no file. Re-exported
by :mod:`state_store` so external callers keep the same import path.
...
too, and every host can see it. One host running ``sac store tick`` would then
...
still returned by :func:`.state_store_instances.last_known_instance`, which is
...
from .state_store_instances import end_instance, list_active_instances
...
def gc_dead_instances(
    *,
    ...
    """
    from .state_store import _resolve_host, now_iso
