...
is what the ACL is DERIVED FROM: :func:`..state_store_nodes.derive_group`
turns these edges into the default-ACL group,
:func:`.state_store_lineage_rel.sender_target_relationship` turns them into
...
may spawn (:func:`..state_store_nodes.spawn_allowed`) — the outage would hand
...
oplog. That maps onto :func:`..state_store_nodes.record_lineage`'s
...
:mod:`.state_store_comms_nodes_store`: ``psycopg.connect`` costs 10.707 ms
...
#: :mod:`.state_store_comms_nodes_store`. These edges are read from
...
def run_with_reconnect(operation: "Callable[[Store], Any]") -> Any:
    ...
    :func:`..state_store_nodes.record_lineage`, is a get-then-put that is
