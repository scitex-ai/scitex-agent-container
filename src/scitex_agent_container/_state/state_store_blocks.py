...
* UNBLOCK is the existing :func:`state_store_nodes.grant_send` — writes the
...
``revoke_send`` / ``has_grant`` shape in ``state_store_nodes``.
...
def ensure_comms_blocks_table() -> str:
    ...
    Kept under its original name because :func:`_state.state_store.init_schema`
