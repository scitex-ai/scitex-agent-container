...
Lives in a sibling module so :mod:`.state_store_nodes` stays under the
per-file line cap. Re-exported from ``state_store_nodes`` so the natural
...
* **The default-ACL mesh keeps ONE bucket per agent — first-of.** "Do
  ...
  :func:`.state_store_nodes.resolve_group_name` (the primary group),
...
def resolve_group_names(
    ...
    **The spec answers first** (operator 2026-08-12, "configuration →
    ...
    bare host. The retired SQLite cache could not do that: its private
    per-agent shard held no policy row for anyone, so this function returned the
...
def is_researcher(
    *,
    ...
    :func:`.state_store_nodes.spawn_allowed`.
    """
