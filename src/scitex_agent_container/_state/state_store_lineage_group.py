...
Extracted from :mod:`.state_store_nodes` on 2026-08-28, when ``lineage`` moved
...
:mod:`.state_store_lineage_rel` its own file: ``state_store_nodes`` is the import
...
Re-exported from :mod:`.state_store_nodes`, so every existing
...
def record_lineage(
    ...
    **IMMUTABLE keeps the first value and does NOT raise.** A differing
    ...
    from .state_store_lineage_store import ACTOR, run_with_reconnect
