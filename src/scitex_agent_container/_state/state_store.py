"""The ``state_store`` facade — two id helpers and the accessor re-exports.
...
``from ...state_store import X`` call site still resolves through this name.

The accessors themselves each own their storage now: :mod:`state_store_diary`
(``turns`` / ``errors`` / ``heartbeats``), :mod:`state_store_instances`
(``instances``), :mod:`state_store_gc` and :mod:`state_store_export`. This module
...
# Re-exported for the 7 modules that ``from ...state_store import
# _resolve_host`` (claude_session, _node_channel, state_store_export,
# state_store_gc, _send, send_cmds, _dispatch).
from .state_store_hostname import resolve_host as _resolve_host  # noqa: F401
...
# keep importing them from :mod:`state_store`.
from .state_store_diary import (  # noqa: E402,F401
    latest_heartbeats_per_name,
    record_error,
    record_heartbeat,
    record_turn,
)
from .state_store_export import import_legacy_registry  # noqa: E402,F401
from .state_store_gc import (  # noqa: E402,F401
    _proc_btime,
    gc_dead_instances,
)
