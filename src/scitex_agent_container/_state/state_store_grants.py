...
Extracted from :mod:`.state_store_nodes` so that module stays under the
per-file line cap. The four primitives below are re-exported from
``state_store_nodes`` so the existing import surface is unchanged:
...
THE SCHEMA AND THE CONNECTION MOVED TO :mod:`.state_store_grants_store` on
...
from .state_store_grants_store import (
    ACTOR,
    GRANTS_STORE,
    grant_as_dict,
    grants_schema,
