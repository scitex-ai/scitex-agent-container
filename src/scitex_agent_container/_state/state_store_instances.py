#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# File: src/scitex_agent_container/_state/state_store_instances.py
...
:mod:`.state_store`, so every ``from ...state_store import record_instance_start``
keeps resolving. What changed is underneath: the rows live in the shared
PostgreSQL store (:mod:`.state_store_instances_store`), not in a per-host
file, and ``db_path`` is GONE from every signature because there is
no file to point at.

The agent-rename half lives in :mod:`.state_store_instances_rename` — the
...
1. **A stop reads first and refuses explicitly.** The tombstone was
   ...
   as :func:`.state_store_incarnations.record_incarnation_exit`.
...
2. **Every write is a PARTIAL put, and every payload is stripped of
   ...
   ``MergeConflict``. See :func:`.state_store_instances_store.strip_unset`,
...
arbitrary one. :func:`.state_store_instances_store.sortable_recency` is the one
...
def record_instance_start(
    ...
    *,
    ...
    :mod:`.state_store_forward`). Callers may keep passing either or both;
    ...
    from .state_store import new_uuid7, now_iso
...
# ``state_store_gc``'s staleness branch reads the first of them. With no
...
def live_instance_for_name(name: str) -> dict | None:
    ...
    The shared reader behind :func:`.state_store_nodes.resolve_node_host` and
    :func:`.state_store_forward.resolve_forward_target`, which spelled the same
