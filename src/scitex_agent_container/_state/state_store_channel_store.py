...
Split out of :mod:`.state_store_channel` (which stays the import surface every
...
from :mod:`.state_store_comms_nodes_store`, which arrived at each piece by
...
#: ``state_store_comms_nodes_store.DEFAULT_CONNECT_TIMEOUT_S``: this store is
...
#: The two tables. Declared here rather than in ``state_store_schema`` because
...
#: :func:`.state_store_channel.mark_delivered` had to grow a ``target``
#: argument.
#:
...
def _with_connect_timeout(target: Any) -> Any:
    ...
    Copied from :mod:`.state_store_comms_nodes_store`. The bound has to travel
...
def _is_connection_lost(exc: BaseException) -> bool:
    ...
    Copied from :mod:`.state_store_comms_nodes_store`. The distinction decides
...
#: theoretical race.
#:
#: This is the SAME arrangement ``state_store_comms_nodes_store`` relies on — its
...
def run_with_reconnect(operation: "Callable[[psycopg.Connection], Any]") -> Any:
    ...
    WHY THIS EXISTS, MEASURED (``state_store_comms_nodes_store``, 2026-08-28)
    =====================================================================
...
def new_channel_connection() -> "psycopg.Connection":
    ...
    :mod:`.state_store_channel` states its own atomicity, and an implicit
