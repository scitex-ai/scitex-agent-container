...
the prior limited scope had deferred):
  ...
  * **Authenticated identity** — ``mint_node_token`` /
    ...
    (from ``state_store_node_tokens``) until 2026-08-28, when the per-node
  ...
  * **Two group projections, one source.** ``group_name`` is the
    ...
    :mod:`.state_store_groups`). Both are written from the same
...
from .state_store_acl_policy import (
...
# :mod:`.state_store_lineage_group` on 2026-08-28, when ``lineage`` left
...
# groups", NOT "is it the primary group" — see state_store_groups for why
# that distinction is the whole point (incident 2026-08-10, grant).
from .state_store_groups import (  # noqa: E402
    in_named_group,
    is_developer,
    is_privileged,
    is_researcher,
...
def spawn_allowed(
    ...
    **Per-spec override still applies**: every allow path here flows
    ...
    from .state_store_lineage_store import parent_name_of
...
def _spawn_denied_reason(caller: str, parent: str) -> str:
    ...
    """
    from .state_store_acl_policy import comms_policy_row_exists
...
# in a sibling module (state_store_grants) under the per-file line cap; re-exported
# here so the natural import path
# ``from ..._state.state_store_nodes import grant_send`` keeps working.
# ---------------------------------------------------------------------------

from .state_store_grants import (  # noqa: E402, F401
    grant_send,
    has_grant,
    list_comms_grants,
    revoke_send,
...
def resolve_node_host(*, name: str) -> dict[str, Any] | None:
    ...
    if not name:
        return None
    from .state_store_comms_nodes import resolve_comms_node_host
    from .state_store_instances import live_instance_for_name
...
# module (state_store_comms_nodes) under the per-file line cap; re-exported
# here so the natural import path
# ``from ..._state.state_store_nodes import register_comms_node`` works.
# ---------------------------------------------------------------------------

from .state_store_comms_nodes import (  # noqa: E402, F401
    CommsNodeConflictError,
    list_comms_nodes,
    lookup_comms_node,
    register_comms_node,
