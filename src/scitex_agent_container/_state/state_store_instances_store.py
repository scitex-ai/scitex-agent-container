#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# File: src/scitex_agent_container/_state/state_store_instances_store.py
"""The ``instances`` store — schema, connection, and the row codec.

Split out of :mod:`.state_store_instances` (which stays the import surface
:mod:`.state_store` re-exports) to keep both files under the per-file line cap,
exactly as :mod:`.state_store_comms_nodes_store` is split out of its sibling.
...
which to prefer, and ``state_store_forward``'s docstring records a live row on
...
def instances_schema() -> Any:
    ...
    return Schema(
        ...
        fields={
            ...
            "heartbeat_agent_id": data(FieldKind.TEXT, lww),
