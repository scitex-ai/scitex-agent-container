...
``host:a2a_port``". :func:`..state_store_nodes.resolve_node_host` consults it
when no live ``instances`` row matches, which is what lets a Spartan agent
address ``lead`` at all.

Public symbols are re-exported from :mod:`state_store_nodes` so callers keep
using ``from ..._state.state_store_nodes import register_comms_node``.

ON POSTGRESQL SINCE 2026-08-28. The schema, the ``Store`` factory and the
row codec live in :mod:`.state_store_comms_nodes_store`, whose docstring
...
from .state_store_comms_nodes_store import (
    ACTOR,
    COMMS_NODES_STORE,
    comms_node_as_dict,
    comms_nodes_schema,
...
def resolve_comms_node_host(*, name: str) -> dict[str, Any] | None:
    ...
    :func:`..state_store_nodes.resolve_node_host` shape) or ``None`` when the
