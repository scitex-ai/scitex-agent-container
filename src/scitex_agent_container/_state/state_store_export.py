...
along with the ``sac store export`` / ``sac store import`` commands they backed.
...
def import_legacy_registry(
    ...
    but the parameter stays in the signature because ``sac store
    ...
    from .state_store_instances_store import ACTOR, run_with_reconnect, strip_unset
