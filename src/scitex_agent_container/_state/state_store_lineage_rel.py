...
Extracted from :mod:`.state_store_acl_policy` on 2026-08-28, when the policy
...
that consumes it. :mod:`.state_store_acl_policy` answers "what is this agent
allowed to do"; this answers "what IS this agent to that one".

Re-exported from :mod:`.state_store_acl_policy` (and thence from
:mod:`.state_store_nodes`) so every existing import path keeps working.
"""
