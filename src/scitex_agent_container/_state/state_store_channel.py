...
and connection in :mod:`.state_store_channel_store`):
...
summarised in :mod:`.state_store_channel_store`.
...
4. ``meta_json`` round-trips BYTE-IDENTICALLY. See
   :mod:`.state_store_channel_store` for why the column is ``TEXT`` and never
...
from .state_store_channel_store import run_with_reconnect
