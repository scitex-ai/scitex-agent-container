...
def test_an_unknown_agent_is_started_rather_than_no_opped(tmp_path, registry, pg_schema: str):
...
def test_an_unknown_agent_is_started_without_force(tmp_path, registry, pg_schema: str):
...
def test_a_dead_agent_is_started(tmp_path, registry, pg_schema: str):
    # Arrange
    spec = _write_spec(tmp_path)
    registry.add("alpha", str(spec), "cld-alpha")
...
def test_an_alive_agent_still_no_ops(tmp_path, registry, pg_schema: str):
...
def test_an_alive_agent_no_op_returns_success(tmp_path, registry, pg_schema: str):
    # Arrange
    spec = _write_spec(tmp_path)
    registry.add("alpha", str(spec), "cld-alpha")
    runtime = _Runtime(running=True, start_result=True)
...
def test_an_alive_no_op_announces_agent_and_session_loudly(
    tmp_path, registry, caplog, pg_schema: str
):
