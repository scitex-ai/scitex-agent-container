...
def test_an_unknown_agent_is_started_rather_than_no_opped(pg_schema, tmp_path, registry):
...
def test_an_unknown_agent_is_started_without_force(pg_schema, tmp_path, registry):
...
def test_a_dead_agent_is_started(pg_schema, tmp_path, registry):
    # Arrange
    spec = _write_spec(tmp_path)
    registry.add("alpha", str(spec), "cld-alpha")
...
def test_an_alive_agent_still_no_ops(pg_schema, tmp_path, registry):
...
def test_an_alive_agent_no_op_returns_success(pg_schema, tmp_path, registry):
    # Arrange
    spec = _write_spec(tmp_path)
    registry.add("alpha", str(spec), "cld-alpha")
    runtime = _Runtime(running=True, start_result=True)
