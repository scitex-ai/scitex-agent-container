...
def test_allow_stale_spec_starts_the_agent(tmp_path, registry, pg_schema: str):
...
def test_allow_stale_spec_env_starts_the_agent(tmp_path, registry, env_save_restore, pg_schema: str):
...
def test_unpushed_local_commits_still_start(tmp_path, registry, pg_schema: str):
...
def test_unpushed_local_commits_still_warn(tmp_path, registry, capsys, pg_schema: str):
...
def test_clean_source_starts_normally(tmp_path, registry, pg_schema: str):
    # Arrange
    spec = _make_spec_repo(tmp_path, drifted=False)
    runtime = _FakeRuntime()
    # Act
