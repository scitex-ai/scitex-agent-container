"""Tests for ``sac store`` group console output.

Coverage closure for ``scitex_agent_container.cli_pkg.store_group``.
...
def test_store_migrate_resolves_registry_dir_from_env_variable(
    ...
    try:
        # Act
        result = runner.invoke(store_migrate, ["--host", "h", "--json"])
...
def test_store_migrate_console_output_reports_imported_count(
    db_path: Path, tmp_path: Path
):
    # Arrange
    reg = tmp_path / "reg"
    ...
    runner = CliRunner()
    # Act
    result = runner.invoke(store_migrate, ["--registry-dir", str(reg), "--host", "h"])
    # Assert
...
def test_store_clean_console_output_emits_swept_label_header(
    ...
    runner = CliRunner()
    # Act
    result = runner.invoke(store_clean, [])
    # Assert
    assert "swept=" in result.output


def test_store_clean_dry_run_console_uses_would_sweep_label(
    db_path: Path, dead_pid_environment
):
    # Arrange
    from scitex_agent_container._state.state_store import record_instance_start
    ...
    runner = CliRunner()
    # Act
    result = runner.invoke(store_clean, ["--dry-run"])
    # Assert
    assert "would-sweep=" in result.output


def test_store_clean_console_lists_crashed_counter_when_nonzero(
    db_path: Path, dead_pid_environment
):
    # Arrange
    from scitex_agent_container._state.state_store import record_instance_start
