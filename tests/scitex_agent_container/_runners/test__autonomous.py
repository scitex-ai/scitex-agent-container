...
@pytest.mark.asyncio
async def test_autonomous_loop_exits_on_drive_until_match():
    inbox: asyncio.Queue = asyncio.Queue()
    stop = asyncio.Event()
    loop = asyncio.get_running_loop()

    async def consumer():
        # First reply does not match; second reply does.
...
@pytest.mark.asyncio
async def test_autonomous_loop_caps_at_max_turns():
    inbox: asyncio.Queue = asyncio.Queue()
    stop = asyncio.Event()
    loop = asyncio.get_running_loop()
    seen: list[str] = []
    ...
    async def consumer():
        # Always reply without the sentinel; loop must exit non-zero
        # at max_turns.
        for _ in range(3):
...
@pytest.mark.asyncio
async def test_autonomous_loop_stops_when_event_set_before_loop_starts():
    inbox: asyncio.Queue = asyncio.Queue()
    stop = asyncio.Event()
    stop.set()
    loop = asyncio.get_running_loop()
