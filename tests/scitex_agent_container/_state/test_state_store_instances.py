...
def test_the_legacy_family_tree_migration_is_gone() -> None:
    ...
    # function in ``state_store_migrations``, so the whole module went with it
    ...
    try:
        importlib.import_module(
            "scitex_agent_container._state.state_store_migrations"
        )
...
def test_bound_port_is_folded_rather_than_declared_twice() -> None:
    ...
    # had: ``state_store_forward`` records a live row where the split answered
...
def test_an_unrelated_later_write_does_not_freeze_ended_at(pg_schema: str) -> None:
    ...
    from scitex_agent_container._state.state_store_instances_rename import (
...
def test_both_resolvers_break_a_same_second_tie_the_same_way(
    ...
    # ``state_store_forward``'s docstring records as a live routing defect.
    from scitex_agent_container._state.state_store_forward import (
...
def test_a_late_heartbeat_cannot_rewind_the_recorded_one(pg_schema: str) -> None:
    ...
    # hold real values and ``state_store_gc``'s staleness branch reads them, so
