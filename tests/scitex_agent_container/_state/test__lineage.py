...
by the lineage-scoped ACL gate. Real PostgreSQL via the ``pg_schema``
fixture (no mocks, PA-306). AAA + one-assert (PA-307).

THE ISOLATION FIXTURE CHANGED WITH THE STORAGE. Every test here used to
take a ``db_path`` naming a per-test SQLite file. The edges moved to one
shared PostgreSQL store, where a second FILE is not a thing that exists,
so that parameter would have isolated nothing while still looking like
it did. ``pg_schema`` gives each test its own throwaway schema, which is
the same guarantee expressed in the storage that is actually there.
"""
...
def test_ancestors_cycle_is_bounded_by_max_depth(pg_schema: str) -> None:
    # Arrange — x's parent is y and y's parent is x. The old version of
    # this test reached past the API to INSERT the second edge, on the
    # stated belief that "record_lineage prevents cycles". It does not:
    # keep-first-parent is per CHILD, so two calls naming two different
    # children are both accepted and close the loop. Measured while
    # porting this file. The raw INSERT was never needed, and the SQLite
    # table it wrote to no longer exists.
