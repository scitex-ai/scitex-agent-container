...
PostgreSQL (:mod:`scitex_agent_container._state.state_store_diary`). The
...
by ``sac store query``, because a table that still exists and is never
...
from scitex_agent_container._state.state_store_diary import (
    ERRORS_STORE,
    HEARTBEATS_STORE,
    TURNS_STORE,
)
...
def _diary_rows(store_name: str) -> list[dict]:
    ...
    Reading through ``state_store_diary._open`` keeps the reader and the writer
    ...
    """
    from scitex_agent_container._state import state_store_diary as diary
...
def test_insert_turn_round_trips_status_field(db_path: Path, pg_schema: str):
    # Arrange
    from scitex_agent_container._state.state_store import record_turn
...
def test_insert_error_round_trips_cause_field(db_path: Path, pg_schema: str):
    # Arrange
    from scitex_agent_container._state.state_store import record_error
...
# ``sac store query --where=...``. The lead expects these filters to work.
# ---------------------------------------------------------------------------
...
class _FakeDBWriter:
    """In-memory recorder substituted for the real ``state_store_diary`` calls."""
