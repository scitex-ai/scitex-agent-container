...
def test_export_state_tables_filter_emits_the_named_tables_rows(
    ...
    # Arrange
    # ``record_instance_start`` seeded the SECOND table here until
    # 2026-08-28, when ``instances`` moved to the shared store and left
    # KNOWN_TABLES. There is no second SQLite table left to seed, so the
    # "excludes unlisted" half below asserts the REFUSAL instead.
    from scitex_agent_container._state.state_db import export_state
...
def test_export_state_tables_filter_excludes_unlisted_tables(
    ...
    # Act
    payload = export_state(tables=["channel_events"])
    # Assert — the payload carries the ONE known table and nothing else.
    assert set(payload["tables"]) == {"channel_events"}
...
def test_db_export_tables_flag_rejects_instances_now_that_it_moved(
    ...
    # shared store and left KNOWN_TABLES, which leaves ONE known table — so
    # there is no second name for a CSV to carry, and the honest assertion is
    # the refusal. It is also the refusal with the widest blast radius: an
    # empty ``instances`` array in an export reads as "no agent has ever run
    # on this host" while PostgreSQL holds the fleet's whole lifecycle
    # history.
    from scitex_agent_container.cli_pkg.db_group import db_export
