...
from scitex_agent_container._state.state_store_lineage_store import read_edges
...
# ---------------------------------------------------------------------------
# ``test_lineage_table_is_absent_from_a_fresh_state_store`` WAS HERE, and of all
...
# what a grant MEANS is covered by test_state_store_grants.py against the store
...
#
# These two moved here from test_state_store_acl_policy.py on 2026-08-28. They
...
# store-natively in test_state_store_grants.py: has_grant false when none /
...
# ``state_store.open_db``, i.e. into the abandoned table, which is why
# they read back empty rather than failing loudly.
#
# WHAT STAYS IS THE ONE THING THIS FILE UNIQUELY COVERS: the four primitives
# are RE-EXPORTED from state_store_nodes, and production imports them from HERE
...
def test_the_grant_primitives_are_importable_from_state_store_nodes() -> None:
    """The re-export is load-bearing: production imports them from here.

    Deliberately NOT a behaviour test — those live in test_state_store_grants.py.
    This asserts only the import surface that state_store_grants.py's own
    ...
    missing = [
        ...
        if not callable(getattr(state_store_nodes, n, None))
    ]
    # Assert
    assert missing == []
