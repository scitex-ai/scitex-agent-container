...
storage engine was deleted from ``state_store``. Both had already been hollowed
...
``sac store show`` and ``sac store query`` were covered here until 2026-08-29, when
both verbs were deleted with the read surface they wrapped.

TQ cleanup (state_store slice): every test carries AAA markers and exactly
...
from scitex_agent_container._state.state_store import import_legacy_registry
...
def _swept_rows() -> list[dict]:
    ...
    """
    from scitex_agent_container._state.state_store_instances import scan_instances
...
# ---------------------------------------------------------------------------
# CLI surface — `sac store migrate`
# ---------------------------------------------------------------------------


def test_store_migrate_via_cli_reports_one_imported_for_single_shard(
    db_path: Path, tmp_path: Path
):
    # Arrange
    reg = tmp_path / "registry"
    _write_legacy_shard(reg, "diag-test")
    from scitex_agent_container.cli_pkg.store_group import store_migrate
...
# together with ``state_store_heartbeats`` — the module that WAS its API — on
...
# ``output_tokens``, and ``state_store_gc``'s heartbeat-staleness rule reads
...
def dead_pid_environment():
    ...
    saved_btime = state_store._proc_btime
    os.environ["SAC_HOST"] = "test-host"
    state_store._proc_btime = lambda: None  # type: ignore[assignment]
    try:
        yield
    finally:
        state_store._proc_btime = saved_btime  # type: ignore[assignment]
...
def test_store_clean_via_cli_reports_at_least_one_crashed_in_json_body(
    db_path: Path, dead_pid_environment
):
    # Arrange
    from scitex_agent_container._state.state_store import record_instance_start
    from scitex_agent_container.cli_pkg.store_group import store_clean

    record_instance_start("dead", pid=999_999_999, host="test-host")
    runner = CliRunner()
    # Act
    result = runner.invoke(store_clean, ["--json"])
    body = json.loads(result.stdout)
    # Assert
    assert body["crashed"] >= 1


def test_store_tick_via_cli_emits_no_human_readable_output_on_success(
    db_path: Path,
):
    # Arrange
    from scitex_agent_container.cli_pkg.store_group import store_tick

    runner = CliRunner()
    # Act
    result = runner.invoke(store_tick, [])
