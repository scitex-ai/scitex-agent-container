...
Covers :mod:`scitex_agent_container._state.state_store_comms_nodes` and the
...
claims that are actually sac's:
  ...
  * ``test_comms_nodes_is_gone_from_a_fresh_state_store`` — a state.db created
...
sibling file ``test_state_store_export_comms_nodes_sync.py`` opened TWO
...
def test_the_declared_schema_has_exactly_the_four_fields() -> None:
    # Arrange
    from scitex_agent_container._state.state_store_comms_nodes_store import (
        comms_nodes_schema,
    )
