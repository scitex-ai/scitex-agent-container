...
These pin the contract of the ``_isolate_state_store`` guard in
...
# ``test_default_db_path_is_not_the_real_state_store`` was here until 2026-08-30.
# It asserted ``state_store.DEFAULT_DB_PATH != ~/.scitex/.../state.db``, and by the
...
def test_state_store_env_var_is_not_the_real_state_store() -> None:
...
def test_a_fresh_state_store_starts_with_an_empty_instances_table(
    pg_schema: str,
) -> None:
