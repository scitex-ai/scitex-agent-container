...
def test_agent_with_live_session_is_not_restarted(pg_schema: str, registry, db_path, history, events):
...
def test_agent_with_live_session_reports_ok(pg_schema: str, registry, db_path, history, events):
    # Arrange
    write_spec(registry, "alpha")
    ghost("alpha")
    # Act
...
def test_live_session_outranks_a_crashed_row(pg_schema: str, registry, db_path, history, events):
...
def test_ghost_active_row_agent_is_restarted(pg_schema: str, registry, db_path, history, events):
...
def test_ghost_active_row_is_reported_restarted(pg_schema: str, registry, db_path, history, events):
    # Arrange
    write_spec(registry, "alpha")
    ghost("alpha")
    # Act
...
def test_whole_dead_fleet_is_recovered(pg_schema: str, registry, db_path, history, events):
...
def test_crashed_agent_is_restarted(pg_schema: str, registry, db_path, history, events):
...
def test_reboot_swept_agent_is_restarted(pg_schema: str, registry, db_path, history, events):
...
def test_always_policy_is_enforced(pg_schema: str, registry, db_path, history, events):
...
def test_deliberately_stopped_agent_is_not_restarted(pg_schema: str, registry, db_path, history, events):
...
def test_deliberately_stopped_agent_reports_skipped(pg_schema: str, registry, db_path, history, events):
    # Arrange
    write_spec(registry, "alpha")
    ended("alpha", "stopped")
    # Act
...
def test_stopped_agent_skip_states_its_reason(pg_schema: str, registry, db_path, history, events):
...
def test_deleted_agent_is_not_restarted(pg_schema: str, registry, db_path, history, events):
    # Arrange
    write_spec(registry, "alpha")
    ended("alpha", "deleted")
    recorder = Recorder()
...
def test_stopped_agent_records_no_verdict(pg_schema: str, registry, db_path, history, events):
...
def test_unmanaged_spec_is_not_restarted(pg_schema: str, registry, db_path, history, events):
...
def test_never_started_spec_is_not_started(pg_schema: str, registry, db_path, history, events):
...
def test_remote_agent_is_not_restarted_locally(pg_schema: str, registry, db_path, history, events):
...
def test_dry_run_restarts_nothing(pg_schema: str, registry, db_path, history, events):
    # Arrange — a corpse that WOULD be restarted under --apply.
    write_spec(registry, "alpha")
    ghost("alpha")
    recorder = Recorder()
    # Act — apply defaults to False.
    run_pass(registry, history, events, restart_fn=recorder)
    # Assert — the irreversible act was never invoked.
    assert recorder.names == []


def test_dry_run_reports_would_restart(pg_schema: str, registry, db_path, history, events):
...
def test_dry_run_records_no_restart(pg_schema: str, registry, db_path, history, events):
...
def test_dry_run_records_no_down_verdict(pg_schema: str, registry, db_path, history, events):
...
def test_blind_probe_restarts_nothing(pg_schema: str, registry, db_path, history, events):
...
def test_blind_probe_reports_unknown(pg_schema: str, registry, db_path, history, events):
    # Arrange — UNKNOWN must never be rendered as clean.
    write_spec(registry, "alpha")
    ghost("alpha")
    # Act
    outcome = run_pass(
    registry, history, events, apply=True, snapshot_fn=lambda **_: None
    )
    # Assert
...
def test_raising_probe_is_unknown_not_dead(pg_schema: str, registry, db_path, history, events):
...
def test_tmux_is_probed_once_per_pass(pg_schema: str, registry, db_path, history, events):
    ...
    for name in ("a1", "a2", "a3", "a4", "a5"):
        ...
        ghost(name)
    # Act
    run_pass(registry, history, events, snapshot_fn=_counting)
    # Assert
    assert len(calls) == 1
...
def test_unreadable_spec_does_not_abort_the_sweep(pg_schema: str, registry, db_path, history, events):
...
def test_missing_registry_is_not_a_crash(registry, db_path, history, events, tmp_path):
    ...
    # Act
    outcome = run_pass(tmp_path / "nope", history, events)
    # Assert
    assert outcome.reports == ()
