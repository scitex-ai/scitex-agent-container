...
def test_debounce_blocks_a_second_restart(pg_schema: str, registry, db_path, history, events):
    ...
    recorder = Recorder()
    # Act
    run_pass(registry, history, events, apply=True, restart_fn=recorder)
    # Assert
    assert recorder.names == []


def test_debounced_agent_reports_cooling_down(pg_schema: str, registry, db_path, history, events):
    ...
    history.write_text(f'{{"alpha": [{NOW - 600}]}}')
    # Act
    outcome = run_pass(registry, history, events, apply=True)
    # Assert
    assert verdict_of(outcome, "alpha") is Verdict.COOLING_DOWN


def test_debounced_agent_records_no_verdict(pg_schema: str, registry, db_path, history, events):
...
def test_second_pass_inside_debounce_does_not_rebounce(
    pg_schema: str, registry, db_path, history, events
):
...
def test_restart_resumes_after_the_debounce_elapses(pg_schema: str, registry, db_path, history, events):
...
def test_over_budget_agent_is_not_restarted(pg_schema: str, registry, db_path, history, events):
...
def test_over_budget_agent_is_recorded_degraded(pg_schema: str, registry, db_path, history, events):
...
def test_over_budget_verdict_is_reported(pg_schema: str, registry, db_path, history, events):
    # Arrange
    write_spec(registry, "alpha")
    ghost("alpha")
    history.write_text(f'{{"alpha": [{NOW - 3_500}, {NOW - 1_900}]}}')
...
def test_pass_limit_caps_one_tick(pg_schema: str, registry, db_path, history, events):
    ...
    # Act
    run_pass(
    registry, history, events, apply=True, limit=2, restart_fn=recorder
    )
    # Assert
    assert len(recorder.names) == 2


def test_capped_agents_are_reported_not_dropped(pg_schema: str, registry, db_path, history, events):
    # Arrange — the remainder is DEFERRED, not lost, and must be visible.
    for name in ("a1", "a2", "a3"):
        write_spec(registry, name)
        ghost(name)
    # Act
    outcome = run_pass(registry, history, events, apply=True, limit=1)
    # Assert
    assert len(outcome.of(Verdict.CAPPED)) == 2


def test_capped_agent_gets_no_record(pg_schema: str, registry, db_path, history, events):
    # Arrange — CAPPED is retried in 5 minutes; recording it would be noise.
    for name in ("a1", "a2"):
        write_spec(registry, name)
        ghost(name)
    # Act
    run_pass(registry, history, events, apply=True, limit=1)
    # Assert
    assert _subjects(events, SUBJECT_DEGRADED) == []


def test_history_is_persisted_per_restart(pg_schema: str, registry, db_path, history, events):
...
def test_failed_restart_is_reported_failed(pg_schema: str, registry, db_path, history, events):
    # Arrange — the restart ran but reported failure; the agent is still down.
    write_spec(registry, "alpha")
    ghost("alpha")
    # Act
    outcome = run_pass(
    registry, history, events, apply=True, restart_fn=Recorder(ok=False)
    )
    # Assert
    assert verdict_of(outcome, "alpha") is Verdict.FAILED


def test_raising_restart_does_not_abort_the_sweep(pg_schema: str, registry, db_path, history, events):
...
def test_failed_restart_is_recorded_degraded(pg_schema: str, registry, db_path, history, events):
    # Arrange
    write_spec(registry, "alpha")
    ghost("alpha")
    # Act
...
def test_recovered_agent_records_its_recovery(pg_schema: str, registry, db_path, history, events):
...
def test_healthy_fleet_exits_clean(pg_schema: str, registry, db_path, history, events):
    # Arrange
    write_spec(registry, "alpha")
    ghost("alpha")
    # Act
    outcome = run_pass(
    registry, history, events, snapshot_fn=lambda **_: sessions("alpha")
    )
    # Assert
    assert outcome.exit_code() == 0


def test_down_agent_exits_nonzero(pg_schema: str, registry, db_path, history, events):
    # Arrange — cron-friendly: a dry run detecting a corpse must exit != 0.
    write_spec(registry, "alpha")
    ghost("alpha")
    # Act
    outcome = run_pass(registry, history, events)
    # Assert
    assert outcome.exit_code() == 1


def test_blind_pass_exits_two(pg_schema: str, registry, db_path, history, events):
    # Arrange — a pass that could not see the fleet must NOT exit 0 and let
    # a cron log it as a healthy tick. Unknown is not clean.
    write_spec(registry, "alpha")
    ghost("alpha")
    # Act
    outcome = run_pass(registry, history, events, snapshot_fn=lambda **_: None)
    # Assert
    assert outcome.exit_code() == 2


def test_successful_recovery_exits_clean(pg_schema: str, registry, db_path, history, events):
...
def test_clean_pass_still_records_the_pass(pg_schema: str, registry, db_path, history, events):
...
def test_dry_run_still_records_the_pass(pg_schema: str, registry, db_path, history, events):
    # Arrange — the beacon is about the RECONCILER, not about an agent, so
    # it ticks in both modes (the record's ``mode`` says which).
    write_spec(registry, "alpha")
    ghost("alpha")
    # Act
    run_pass(registry, history, events)
    # Assert
    assert len(_pass_records(events)) == 1
...
def test_a_record_write_failure_still_restarts(pg_schema: str, registry, db_path, history, unwritable):
...
def test_a_record_write_failure_is_loud(pg_schema: str, registry, db_path, history, unwritable):
...
def test_a_record_write_failure_keeps_the_verdict(pg_schema: str, registry, db_path, history, unwritable):
...
def test_a_pass_record_failure_does_not_stop_restarts(pg_schema: str, registry, db_path, history, unwritable):
